...
export default function DayDrawer({ detail, loading, timeZone, onClose }: { detail: DayDetail | null; loading: boolean; timeZone: string; onClose: () => void }) {
  ...
  return <div className="drawer-backdrop" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}>
    <aside className="drawer">
      <div className="drawer-head"><div><small>DAY DETAIL</small><h3>{detail?.day || '读取中…'}</h3></div><button className="icon-btn" aria-label="关闭当天记录" onClick={onClose}><X size={18} /></button></div>
      ...
      {detail && <>
        ...
        <div className="submission-list">
          ...
          {detail.items.map((item) => <article key={`${item.platform}-${item.account}-${item.submission_id}`}>
