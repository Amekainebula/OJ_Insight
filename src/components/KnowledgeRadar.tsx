...
function point(index: number, value: number, count: number, radius = 96) {
  const angle = Math.PI * 2 * index / count - Math.PI / 2;
  const scale = value / 100;
  return `${180 + Math.cos(angle) * radius * scale},${145 + Math.sin(angle) * radius * scale}`;
}
...
export default function KnowledgeRadar({ data, selectedPlatform }: { data: KnowledgeBucket[]; selectedPlatform?: Platform | null }) {
  ...
  return <section className="panel knowledge-panel">
    ...
    <div className="knowledge-body">
      <svg viewBox="0 0 360 290" role="img" aria-label={`${LABELS[active]} 能力雷达图`}>
        {grid.map((points, index) => <polygon className="knowledge-grid" points={points} key={index} />)}
        {rows.map((_, index) => <line className="knowledge-axis" x1="180" y1="145" x2={point(index, 100, rows.length).split(',')[0]} y2={point(index, 100, rows.length).split(',')[1]} key={index} />)}
        ...
        {rows.map((item, index) => { const [x, y] = point(index, 122, rows.length); return <text x={x} y={y} textAnchor="middle" dominantBaseline="middle" key={`label-${item.axis}`}>{item.axis}</text>; })}
      </svg>
