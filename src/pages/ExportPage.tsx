  ...
  const run = async (action: 'save' | 'copy') => {
    ...
    try {
      ...
      const delivered = chart === 'heatmap'
        ? await exportHeatmap(title, snap.daily, until ? Number(range.start.slice(0, 4)) : from, until ? Number(range.end.slice(0, 4)) : to, format, range.start, range.end, action)
        : await exportVisual(title, snap, chart, format, action);
