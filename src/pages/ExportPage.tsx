...
function ExportPreview({ chart, title, period, platform }: { chart: ChartType; title: string; period: string; platform: string }) {
  return <div className="export-preview-sheet"><header><div><small>LAYOUT PREVIEW</small><strong>{title}</strong></div><span>{platform} · {period}</span></header><div className={`export-preview-chart chart-${chart}`}>
