...
import { ExternalLink, KeyRound, Monitor, Palette, Plus, RotateCcw, Save, Type, X } from 'lucide-react';
...
export default function SettingsPage({ accounts, timeZone, onTimeZone, preferences, onPreferences, onSaved, syncing, notify }: Props) {
  ...
  return <>
    ...
    {tab === 'accounts' ? <><section className="settings-intro"><strong>怎么填写？</strong><span>填写个人主页 URL 中的账号标识，不是显示昵称。Cookie 与 API 凭据仅保存在本机。删除或改名会在保存时清理原 ID 的本地记录；同步期间请等待完成后再保存。</span></section><section className="account-panel">{PLATFORM_ORDER.map((platform) => <article className="account-card" key={platform}><div className="account-card-head"><div><span className="platform-monogram" style={{ color: PLATFORM_META[platform].accent }}>{PLATFORM_META[platform].short}</span><div><strong>{PLATFORM_META[platform].name}</strong><small>{PLATFORM_META[platform].accountHint}</small></div></div><div className="account-card-actions">{platform === 'codeforces' && <button onClick={() => void api.openExternal('https://codeforces.com/settings/api')}><KeyRound size={14} />创建 API Key<ExternalLink size={12} /></button>}{PLATFORM_META[platform].secretHint && platform !== 'codeforces' && <button onClick={() => void api.openExternal(platform === 'atcoder' ? 'https://atcoder.jp/login' : platform === 'nowcoder' ? 'https://www.nowcoder.com/login' : platform === 'qoj' ? 'https://qoj.ac/login' : 'https://leetcode.com/accounts/login/')}><KeyRound size={14} />登录获取 Cookie<ExternalLink size={12} /></button>}<button onClick={() => add(platform)}><Plus size={14} />添加 ID</button></div></div><div className="account-inputs">{form[platform]?.map((entry, index) => platform === 'codeforces' ? (() => { const credentials = parseCodeforcesCredentials(entry.secret); return <div className="account-entry account-entry-cf" key={index}><span>{index + 1}</span><input aria-label={`Codeforces ID ${index + 1}`} value={entry.account} onChange={(event) => change(platform, index, 'account', event.target.value)} placeholder={PLATFORM_META[platform].accountHint} /><input aria-label={`Codeforces API Key ${index + 1}`} type="password" value={credentials.apiKey} onChange={(event) => changeCodeforcesCredential(index, 'apiKey', event.target.value)} placeholder="API Key（可选）" autoComplete="off" /><input aria-label={`Codeforces API Secret ${index + 1}`} type="password" value={credentials.apiSecret} onChange={(event) => changeCodeforcesCredential(index, 'apiSecret', event.target.value)} placeholder="API Secret（可选）" autoComplete="off" /><input aria-label={`Codeforces Cookie ${index + 1}`} type="password" value={credentials.cookie} onChange={(event) => changeCodeforcesCredential(index, 'cookie', event.target.value)} placeholder="Cookie（读取提交代码，可选）" autoComplete="off" /><button className="remove-account" onClick={() => remove(platform, index)} aria-label="移除账号"><X size={14} /></button></div>; })() : <div className="account-entry" key={index}><span>{index + 1}</span><input aria-label={`${PLATFORM_META[platform].name} ID ${index + 1}`} value={entry.account} onChange={(event) => change(platform, index, 'account', event.target.value)} placeholder={PLATFORM_META[platform].accountHint} />{PLATFORM_META[platform].secretHint ? <input aria-label={`${PLATFORM_META[platform].name} Cookie ${index + 1}`} type="password" value={entry.secret} onChange={(event) => change(platform, index, 'secret', event.target.value)} placeholder={PLATFORM_META[platform].secretHint} autoComplete="off" /> : <small>公开数据，无需密码</small>}<button className="remove-account" onClick={() => remove(platform, index)} aria-label="移除账号"><X size={14} /></button></div>)}</div></article>)}</section><PersonalInfoImport accounts={PLATFORM_ORDER.flatMap((platform) => form[platform]).filter((entry) => entry.account.trim())} onImported={onSaved} notify={notify} /><PersonalInfoExport accounts={PLATFORM_ORDER.flatMap((platform) => form[platform])} notify={notify} /></> : <section className="preferences-panel">
      <article className="panel preference-card"><div className="preference-heading"><Palette size={18} /><div><small>APPEARANCE</small><h2>外观</h2><p>修改后立即生效，并保存在当前设备。</p></div></div><div className="preference-row"><div><strong>主题</strong><span>灰色介于亮色与暗色之间；跟随系统仍响应操作系统的亮暗模式。</span></div>{choices(preferences.theme, [['system', '跟随系统'], ['light', '亮色'], ['gray', '灰色'], ['dark', '暗色']], (theme) => onPreferences({ theme }))}</div><div className="preference-row"><div><strong>字号</strong><span>标准字号已经提高小字尺寸，不会恢复到原来的过小字号。</span></div>{choices(preferences.fontSize, [['standard', '标准'], ['large', '放大'], ['xlarge', '特大']], (fontSize) => onPreferences({ fontSize }))}</div><div className="preference-row"><div><strong>界面密度</strong><span>只调整卡片和列表间距，不缩小文字。</span></div>{choices(preferences.density, [['comfortable', '舒适'], ['compact', '紧凑']], (density) => onPreferences({ density }))}</div></article>
